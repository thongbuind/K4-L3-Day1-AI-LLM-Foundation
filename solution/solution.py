...
def call_openai(
    ...
    response = client.chat.completions.create(messages=messages, model=model, temperature=temperature, top_p=top_p, max_completion_tokens=max_tokens)
...
def call_openai_mini(
    ...
    response_text, latency_seconds = call_openai(prompt, model=OPENAI_MINI_MODEL, temperature=temperature, top_p=top_p, max_tokens=max_tokens)
...
def chat_with_system_prompt(
    ...
    start_time = time.perf_counter()
    response = client.chat.completions.create(messages=messages, model=model, temperature=temperature, max_completion_tokens=max_tokens)
    end_time = time.perf_counter()
    latency_seconds = end_time - start_time
    response_text = response.choices[0].message.content
    return response_text, latency_seconds
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    input_cost = input_tokens / 1000 * pricing["input"]
    output_cost = output_tokens / 1000 * pricing["input"]
